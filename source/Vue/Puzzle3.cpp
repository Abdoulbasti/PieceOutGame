...
Puzzle3::Puzzle3(int T ,int C, int L, int ML, int MR, int MT, int MB):TILE_SIZE{T},NB_COL{C},
NB_LIGNE{L},MARGIN_LEFT{ML},MARGIN_RIGHT{MR},MARGIN_TOP{MT},MARGIN_BOTTOM{MB}{}

int Puzzle3::jouerPuzzle3()
{
    ...
    // Création des pièces
    vector<pair<int, int>> coords{{2, 0}, {2, 1}, {2, 2}, {3, 2}};
    PieceConcrete tetris_L(coords);
    coords = {{1, 4}, {2, 4}, {1, 5}, {2, 5}};
    PieceConcrete tetris_O(coords);
    coords = {{4, 3}, {4, 4}, {4, 5}, {3, 5}};
    PieceConcrete tetris_J(coords);
    coords = {{5, 2}, {5, 3}, {5,4}};
    PieceConcrete tetris_I(coords);

    // Décoration des pièces
    Piece* l = new OperateurDeplacement{tetris_L, {2, 0}, OrientationDeplacement::NORD};
    l = new OperateurDeplacement{*l, {2, 2}, OrientationDeplacement::SUD};
    l = new OperateurSymetrie{*l, {2, 1}, OrientationSymetrie::VERTICALE};

    Piece* o = new OperateurDeplacement{tetris_O, {1, 4}, OrientationDeplacement::NORD};
    o = new OperateurDeplacement{*o, {2, 5}, OrientationDeplacement::SUD};
    o = new OperateurDeplacement{*o, {2, 4}, OrientationDeplacement::EST};
    o = new OperateurDeplacement{*o, {1, 5}, OrientationDeplacement::OUEST};

    Piece* j = new OperateurDeplacement{tetris_J, {4, 3}, OrientationDeplacement::NORD};
    j = new OperateurDeplacement{*j, {4, 5}, OrientationDeplacement::SUD};
    j = new OperateurRotation{*j, {4, 4}, OrientationRotation::ANTIHORAIRE};

    Piece* i = new OperateurDeplacement{tetris_I, {5, 2}, OrientationDeplacement::NORD};
    i = new OperateurDeplacement{*i, {5, 4}, OrientationDeplacement::SUD};
    ...
    vector<pair<vector<pair<int, int>>, Piece*>> vecteurGain = {{{{0,0},{1,0},{0,1},{1,1}},o}};
    ...
    D->decoration(scene_particuliere, *o, *G);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left &&
                ...
                if (P->estOperationValide(*l, trig_x, trig_y) || P->estOperationValide(*o, trig_x, trig_y) ||
                    P->estOperationValide(*i, trig_x, trig_y) || P->estOperationValide(*j, trig_x, trig_y)) {
                    l->trigger({trig_x, trig_y});
                    o->trigger({trig_x, trig_y});
