...
int Puzzle2::jouerPuzzle2()
{
    ...
    // Création des pièces
    vector<pair<int, int>> coords{{1, 2}, {1, 3}, {1, 4}, {2, 4}};
    PieceConcrete tetris_L(coords);
    coords = {{3, 2}, {3, 3}, {3, 4}, {2, 3}};
    PieceConcrete tetris_T(coords);
    coords = {{2, 0}, {2, 1},{2, 2}};
    PieceConcrete tetris_I(coords);

    // Décoration des pièces
    Piece* l = new OperateurRotation{tetris_L, {1, 3}, OrientationRotation::ANTIHORAIRE};
    Piece* t = new OperateurDeplacement{tetris_T,{3,2},OrientationDeplacement::NORD};
    t = new OperateurDeplacement{*t,{3,4},OrientationDeplacement::SUD};
    t = new OperateurSymetrie{*t, {3, 3}, OrientationSymetrie::HORIZONTALE};

    Piece* i = new OperateurDeplacement{tetris_I, {2, 0}, OrientationDeplacement::NORD};
    i = new OperateurDeplacement{*i, {2, 2}, OrientationDeplacement::SUD};
    i = new OperateurRotation{*i, {2, 1}, OrientationRotation::ANTIHORAIRE};

    // Création du plateau de jeu
    Plateau* P = new Plateau{NB_COL, NB_LIGNE};

    // Élimination des cases non jouables
    coords = {{0, 0}, {1, 0}, {3, 0}, {4, 0},{4, 1},{4, 3},{4, 4},{4, 6},{4,7},
             {5, 1}, {5, 3}, {5,4}, {0,4}, {0,5},{1,5}, {0, 6}, {1, 6}, {4, 6}, {0, 7}, {1, 7}, {3, 7}, {4, 7}};
    P->initialiserNonJouable(coords);

    // Définition des cases gagnantes
    vector<pair<vector<pair<int, int>>, Piece*>> vecteurGain = {{{{2, 5}, {2, 6},{2,7}}, i}};
